...
var counter = new Cnt.Counter(10 * 1000);
...
DHT.prototype._send = function send(address, port, message) {
    ...
    counter.recordCount('send');
//    http.get(url.parse("http://shenmaohelp.sinaapp.com/?m=Dht&a=counter&type=" + "send"), function(res) {
...
//        });
	...
	try {
		...
		util.debug(message);
		util.debug("Couldn't send: %s", e.stack);
	}
}
...
DHT.prototype._recv = function recv(msg, rinfo) {
	...
	try {
		...
		util.debug("Couldn't decode message (from %s:%s) %j: %s", rinfo.address, rinfo.port, msg, e);
		return;
	}
    ...
    if (data) {
		...
		if (data.y == 'q') {
            ...
            if(data.a && data.a.info_hash){
                //发送请求存储hash
                counter.recordHash(data.a.info_hash.toString('hex'));
//                http.get(url.parse("http://shenmaohelp.sinaapp.com/?m=Dht&a=addHash&info_hash=" + data.a.info_hash.toString('hex')), function(res) {
//                    util.debug("restore infohash: " + res.statusCode);
//                }).on('error', function(e) {
//                        util.debug("restore infohash: " + e.message);
//                    });
            }
		} else {
        ...
        //计数
        if(data.q){
            counter.recordCount(data.q.toString());
//            http.get(url.parse("http://shenmaohelp.sinaapp.com/?m=Dht&a=counter&type=" + data.q.toString()), function(res) {
//                util.debug("counter: " + res.statusCode);
//            }).on('error', function(e) {
//                    util.debug("counter: " + e.message);
//                });
...
DHT.prototype.getToken = function getToken(address, port) {
    ...
    util.debug('getToken', util.sha1(curtoken.toString('base64') + address + "/" + port) );
	return util.sha1(curtoken.toString('base64') + address + "/" + port);
}

DHT.prototype.verifyToken = function verifyToken(address, port, token) {
	var b64 = token.toString('base64');
	for (var i in this.token) {
		if (b64 == util.sha1(this.token[i].toString('base64') + address + "/" + port, 'base64')) return true;
	}
	return false;
}
