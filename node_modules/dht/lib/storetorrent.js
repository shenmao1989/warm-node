    ...
    db.open(function(err,db){
        ...
        if(!err){
            ...
            db.collection('torrents', function(err,collection){
                ...
                collection.ensureIndex({'info_hash':1}, {unique:true, background:true, dropDups:true, w:1}, function(err, indexName) {});
                collection.ensureIndex({'num':1}, {w:1}, function(err, indexName) {});
                collection.ensureIndex({'is_read':1}, {w:1}, function(err, indexName) {});
//                insertTorrent(12445);
//                getTopTorrent(5);
            });
        }
...
//            console.log('insert info_hash ' + info_hash);
    ...
    this.setTorrensReaded = function(infos){
        for(var i = 0; i < infos.length; i++){
            self.collection.update({'info_hash': infos[i]}, {$set:{'is_read': 1}});
        }
    }
}
